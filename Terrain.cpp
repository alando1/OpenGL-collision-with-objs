...
Terrain::Terrain(int len, int wid) :
	length(len), width(wid), tileSize(1), heightRatio(50.0f), scalar(64.0f), 
	vertices(NULL), normals(NULL) , colors(NULL) , numOfVerts(0)
{
	heights = new float[width*length];
	newHeights = new float[width*length];
}
...
void Terrain::normalizeTerrain()
{
	...
	for(int i=0; i < (nSize-1); i++)
	{
		for(int j=0; j < (nSize-1); j++)
		{
			...
			p0.x = scalar*i;
			p0.y = scalar*(heightRatio*heights[idx0]);
			p0.z = scalar*j;

			p1.x = scalar*i;
			p1.y = scalar*(heightRatio*heights[idx1]);
			p1.z = scalar*(j+1);

			p2.x = scalar*(i+1);
			p2.y = scalar*(heightRatio*heights[idx2]);
			p2.z = scalar*(j+1);

			p3.x = scalar*(i+1);
			p3.y = scalar*(heightRatio*heights[idx3]);
			p3.z = scalar*j;
...
void Terrain::buildHeightMap(unsigned char* buffa)
{
	...
	for(int i=0; i < (nSize*nSize); i++)
	{
		...
		//cout << "last j value : " << j << endl;
		newHeights[i] = heights[i] * heightRatio * scalar;
	}
	auto finish = chrono::steady_clock::now();
	dt = chrono::duration_cast<chrono::duration<float, milli>>(finish-start).count();
...
float Terrain::getSample(const Vec3& v)
{
	return getSample(v.x,v.z);
}

float Terrain::getSample(float x, float z)
{
	int tmpzt = static_cast<int>((z)/(scalar));
	int tmpxt = static_cast<int>((x)/(scalar)); 
	if(tmpxt < 0 || tmpxt > 255 || tmpzt < 0 || tmpzt > 255)
	{
	...
	{
		//cout << "newHeight tmpIndx: ";
		int tmpIndx = tmpzt * 256 + tmpxt;	//256*256 mapped pixels
		//cout << newHeights[tmpIndx] << endl;
		return newHeights[tmpIndx];
	}
}

bool Terrain::isInBounds(const Vec3& v)
{
	return isInBounds(v.x,v.z);
}

bool Terrain::isInBounds(float x, float z)
{
	int tmpzt = static_cast<int>((z)/(scalar));
	int tmpxt = static_cast<int>((x)/(scalar)); 
	if(tmpxt < 0 || tmpxt > 255 || tmpzt < 0 || tmpzt > 255)
