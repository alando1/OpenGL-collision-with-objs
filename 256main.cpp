...
list<Obj*> weapons;
list<Item*> items;
list<Obj*>::iterator currentWeapon;
...
void handleFunc(float dt)
{
	...
	if(keyStates['w'] || keyStates['W'])
	{
		...
		camPos.x+=camLook.x*(crouchFactor*camSpeed*200.0f*dt);
		//camPos.y+=camLook.y;	//comment out to walk forward on plane
		camPos.z+=camLook.z*(crouchFactor*camSpeed*200.0f*dt);
	}
	...
	if(keyStates['a'] || keyStates['A'])
	{
		...
		vecy = camLook.y;
		vecz = -camLook.x;
		camPos.x+= vecx*(crouchFactor*camSpeed*200.0f*dt);
		camPos.z+= vecz*(crouchFactor*camSpeed*200.0f*dt);
	}
...
void renderScene(void)
{
	...
	if(hasPistol == FALSE)
	{
		...
		glTranslatef(portalGunPos.x + 1000, 14825, portalGunPos.z);
		glScalef(85,85,85);
			pistol->draw();
		glPopMatrix();				
	}
...
void load()
{
	...
	//assault = new Obj("./data/AK47/AK47.obj", "./data/AK47/textures/diffuse.png");
	building = new Obj("./data/City/City.obj", "./data/City/Maps/city.png");
	portalGun = new Obj();

	stormtrooper = new Obj("./data/stormtrooper/Stormtrooper.obj", "./data/stormtrooper/Stormtrooper.png");
	yoda = new Obj("./data/Joda/Joda.obj", "./data/Joda/Joda.png");
	objList.push_back(stormtrooper);
	objList.push_back(yoda);


	yoda->pos = yodaPos;
	stormtrooper->pos = stormPos;
	stormtrooper->setRot(iconSpin/90, 0,1,0);
	stormtrooper->setScale(85.0f, 85.0f, 85.0f);

	tree->setPos(portalGunPos.x - 200.0f, 14825.0f, portalGunPos.z-5000.0f);
	tree->setScale(200.0f,200.0f,200.0f);

	building->setPos(portalGunPos.x, 4825.0f, portalGunPos.z-5000);
	building->setScale(40.0f,40.0f,40.0f);
	...
	iPistol->setScale(85.0f);
	iPistol->setPos(portalGunPos.x + 1000, 14825, portalGunPos.z);

	wPistol->setPos(1.25f, -0.25f, -4.0f);
	wPistol->setRot(90,0,1,0);

	/*----End of Load Object files--------------------------------------------------*/

	/*----Load Texture images-------------------------------------------------------*/
	fire = SOIL_load_OGL_texture("./data/newGun/handgun_Fire.png", SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, SOIL_FLAG_MIPMAPS | SOIL_FLAG_INVERT_Y);
...
void drawFire()
{
	...
	GLfloat vertices[] = { 0, 0, 0, 	0, 0, -16.4f,		0, 12.7f, -16.4f,		0, 12.7f, 0 };
